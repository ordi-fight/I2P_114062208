...
class EnemyTrainer(Entity):
    ...
    def update(self, dt: float) -> None:
        ...
        # print("Enemy detected:", self.detected, " SPACE:", input_manager.key_pressed(pygame.K_SPACE))
        # 若偵測到就直接進入戰鬥（測試用，之後可改回需按 SPACE）
    ...
    def _get_los_rect(self) -> pygame.Rect | None:
        ...
        if self.direction == Direction.UP:
            return pygame.Rect(self.position.x, self.position.y - ts*3, ts, ts*3)
        if self.direction == Direction.DOWN:
            return pygame.Rect(self.position.x, self.position.y + ts, ts, ts*3)
        if self.direction == Direction.LEFT:
            return pygame.Rect(self.position.x - ts*3, self.position.y, ts*3, ts)
        if self.direction == Direction.RIGHT:
            return pygame.Rect(self.position.x + ts, self.position.y, ts*3, ts)
        return None
      

    def _has_los_to_player(self) -> None:
        player = self.game_manager.player
        rect = self._get_los_rect()
