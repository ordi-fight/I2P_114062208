...
#         element.rect.left =  self.player_mon_sprite.rect.right + 50
#         element.rect.top =   self.player_mon_sprite.rect.top + 60
...
class CatchScene(Scene):
  ...
  { "name": "Dragonite", "hp": 180, "max_hp": 250, "attack": 45, "defense": 35, "level": 20, "element": "water", "win_count": 0, "sprite_path": "menu_sprites/menusprite6.png" }
    ...
    def draw(self,screen):
        ...
        if self.is_pokeball:
            self.background.draw(screen)
            pokeball_sprite = Sprite( "ingame_ui/ball.png" ,(20,20))
            pokeball_sprite.rect.center = self.enemy_mon_sprite.rect.center
            pokeball_sprite.draw(screen)
        # draw player monster (left bottom)
